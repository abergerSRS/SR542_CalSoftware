...
filename = r'D:\Documents\MCUXpressoIDE_10.1.0_589\workspace\SR544\tools\edgesAndCounts_80Hz_10100Blade_ticksRescaled.txt'
...
def extractCountDeltas(FTMcount, time):
    ...
    for i, count in enumerate(FTMcount[1:], start=1):
        if count != FTMcount[i-1]:
            t1 = np.append(t1, time[i])
            delta = np.append(delta, deltaCount[i])
...
runningTotal, rescaledTimeAtDelta = extractCountDeltas(deltaCount, t)

# Because the MeasureShaftSpeed() function periodically resets the runningTotal
# FTM count, there will be instances when the rescaledDelta is *not* the difference
# between the currentTotal and the previous total
rescaledDelta = np.zeros(len(runningTotal))
for i, currentTotal in enumerate(runningTotal):
    if currentTotal > runningTotal[i-1]:
        rescaledDelta[i] = currentTotal - runningTotal[i-1]
    else:
        rescaledDelta[i] = currentTotal

# This can be easily converted to delta t in seconds
encDeltaT_sec = encDelta/f_FTM
rescaledDeltaT_sec = rescaledDelta/f_FTM
...
ax1.plot(rescaledTimeAtDelta[1:], rescaledSpeed[1:])
...
ax1.legend(('shaft encoder', 'windowed average, N='+str(windowSize), 'rescaled speed', 'corrected speed'))
